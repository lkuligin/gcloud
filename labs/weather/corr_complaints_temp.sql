...
FROM (
  ...
  FROM
    ...
    GROUP BY date, descriptor)x
    JOIN (
      SELECT DATE(created_date) AS date, COUNT(*) AS all_calls_count
...
ON a.date = b.date
GROUP BY descriptor, temperature
)
GROUP BY descriptor
HAVING  total_complaint_count > 5000 AND
  ABS(corr_pct) > 0.5 AND data_count > 5
ORDER BY ABS(corr_pct) DESC
